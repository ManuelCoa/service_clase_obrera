...
var estadocache = caching.NewCache("Knapsack", 100)
var estadsubject = observer.NewSubject()

func init() {
	estadsubject.AddObserver(updateEstadoCache)
}

// updateCache es una función observadora que actualiza la caché
func updateEstadoCache(url string) {
	estadocache.Delete(url)
}

func GetEstado(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := estadocache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}

	// Obtener datos de la base de datos
	data, err := handlers.Get(c, models.GetEstado)
	if err != nil {
	...
	estadocache.Set(c.OriginalURL(), data, len(c.OriginalURL()))
...
func GetEstadoID(c *fiber.Ctx) error {
	...
	// Obtener datos de la base de datos
	data, err := handlers.GetID(c, models.GetEstadoID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func PostEstado(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.HandlePost(c, models.PostEstado)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}

	// Notificar a los observadores para actualizar la caché
	estadsubject.NotifyObservers(c.OriginalURL())

	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusCreated, "El estado se ha creado con éxito", nil)
}
...
func PutEstado(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.Put(c, models.PutEstado)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "El estado se ha actualizado con éxito", nil)
}

func DeleteEstado(c *fiber.Ctx) error {
	// Procesar los datos
	err := handlers.Delete(c, models.DeleteEstadoID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "El estado se ha eliminado con éxito", nil)
}
