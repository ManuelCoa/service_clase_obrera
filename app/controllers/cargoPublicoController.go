...
var cargoPublicocache = caching.NewCache("Knapsack", 100)
var cargoPublicosubject = observer.NewSubject()

func init() {
	cargoPublicosubject.AddObserver(updateCargoPublicoCache)
}

// updateCache es una función observadora que actualiza la caché
func updateCargoPublicoCache(url string) {
	cargoPublicocache.Delete(url)
}

func GetCargoPublico(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := cargoPublicocache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}

	// Obtener datos de la base de datos
	data, err := handlers.Get(c, models.GetCargo)
	if err != nil {
	...
	cargoPublicocache.Set(c.OriginalURL(), data, len(c.OriginalURL()))

	// Devolver los datos al cliente
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito", data)
}

func GetCargoPublicoID(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := cargoPublicocache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}
	...
	// Obtener datos de la base de datos
	data, err := handlers.GetID(c, models.GetCargoID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func PostCargoPublico(c *fiber.Ctx) error {
	clientIP := c.IP()
...
func PutCargoPublico(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.Put(c, models.PutCargo)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func DeleteCargoPublico(c *fiber.Ctx) error {
	// Procesar los datos
	err := handlers.Delete(c, models.DeleteCargo)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
