...
var parroquiaCache = caching.NewCache("Knapsack", 100)
var parroquiaSubject = observer.NewSubject()

func init() {
	parroquiaSubject.AddObserver(updateParroquiaCache)
}

// updateCache es una función observadora que actualiza la caché
func updateParroquiaCache(url string) {
	parroquiaCache.Delete(url)
}

func GetParroquia(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := parroquiaCache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}

	// Obtener datos de la base de datos
	data, err := handlers.Get(c, models.GetParroquia)
	if err != nil {
	...
	parroquiaCache.Set(c.OriginalURL(), data, len(c.OriginalURL()))
...
func GetParroquiaID(c *fiber.Ctx) error {
	...
	// Obtener datos de la base de datos
	data, err := handlers.GetID(c, models.GetParroquiaID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func PostParroquia(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.HandlePost(c, models.PostParroquia)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}

	// Notificar a los observadores para actualizar la caché
	parroquiaSubject.NotifyObservers(c.OriginalURL())

	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusCreated, "La parroquia se ha creado con éxito", nil)
}
...
func PutParroquia(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.Put(c, models.PutParroquia)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "La parroquia se ha actualizado con éxito", nil)
}

func DeleteParroquia(c *fiber.Ctx) error {
	// Procesar los datos
	err := handlers.Delete(c, models.DeleteParroquia)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "La parroquia se ha eliminado con éxito", nil)
}