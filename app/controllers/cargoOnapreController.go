...
var cargoOnaprecache = caching.NewCache("Knapsack", 100)
var cargoOnapresubject = observer.NewSubject()

func init() {
	cargoOnapresubject.AddObserver(updateCargoOnapreCache)
}

// updateCache es una función observadora que actualiza la caché
func updateCargoOnapreCache(url string) {
	cargoOnaprecache.Delete(url)
}

func GetCargoOnapre(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := cargoOnaprecache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}

	// Obtener datos de la base de datos
	data, err := handlers.Get(c, models.GetCargoOnapre)
	if err != nil {
	...
	cargoOnaprecache.Set(c.OriginalURL(), data, len(c.OriginalURL()))

	// Devolver los datos al cliente
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito", data)
}

func GetCargoOnapreID(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := cargoOnaprecache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}
	...
	// Obtener datos de la base de datos
	data, err := handlers.GetID(c, models.GetCargoOnapreID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func PostCargoOnapre(c *fiber.Ctx) error {
	clientIP := c.IP()
...
func PutCargoOnapre(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.Put(c, models.PutCargoOnapre)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func DeleteCargoOnapre(c *fiber.Ctx) error {
	// Procesar los datos
	err := handlers.Delete(c, models.DeleteCargoOnapre)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
