...
var rolesCache = caching.NewCache("Knapsack", 100)
var rolesSubject = observer.NewSubject()

func init() {
	rolesSubject.AddObserver(updateRolesCache)
}

// updateCache es una función observadora que actualiza la caché
func updateRolesCache(url string) {
	rolesCache.Delete(url)
}

func GetRoles(c *fiber.Ctx) error {
	// Verificar si los datos están en la caché
	if result, exists := rolesCache.Get(c.OriginalURL()); exists {
		return handlers.CreateSuccessResponse(c, fiber.StatusOK, "Datos obtenidos con éxito del cache", result)
	}

	// Obtener datos de la base de datos
	data, err := handlers.Get(c, models.GetRoles)
	if err != nil {
	...
	rolesCache.Set(c.OriginalURL(), data, len(c.OriginalURL()))
...
func GetRolesID(c *fiber.Ctx) error {
	...
	// Obtener datos de la base de datos
	data, err := handlers.GetID(c, models.GetRolesID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
...
func PostRoles(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.HandlePost(c, models.PostRoles)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}

	// Notificar a los observadores para actualizar la caché
	rolesSubject.NotifyObservers(c.OriginalURL())

	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusCreated, "El rol se ha creado con éxito", nil)
}
...
func PutRoles(c *fiber.Ctx) error {
	...
	// Procesar los datos
	err = handlers.Put(c, models.PutRoles)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "El rol se ha actualizado con éxito", nil)
}

func DeleteRoles(c *fiber.Ctx) error {
	// Procesar los datos
	err := handlers.Delete(c, models.DeleteRolesID)
	if err != nil {
		return handlers.CreateErrorResponse(c, fiber.StatusInternalServerError, "Error al procesar la solicitud", err)
	}
	...
	// Devolver una respuesta exitosa
	return handlers.CreateSuccessResponse(c, fiber.StatusOK, "El rol se ha eliminado con éxito", nil)
}